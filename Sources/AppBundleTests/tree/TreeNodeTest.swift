...
final class TreeNodeTest: XCTestCase {
    ...
    func testNormalizeContainers_flattenKeepsTheMostRecentWindows() {
        config.enableNormalizationFlattenContainers = true
        let workspace = Workspace.get(byName: name)
        ...
        TilingContainer.newHTiles(parent: nested, adaptiveWeight: 1).apply {
        ...
        assertEquals(workspace.mostRecentWindowRecursive?.windowId, 1) // Nothing was focused
        assertEquals((nested.mostRecentChild as? Window)?.windowId, 3) // 3 was focused after 2
    }
}
