...
`split` isn't available in the xref:guide.adoc#dwindle-layout[dwindle layout].
Use xref:commands.adoc#dwindle[`dwindle preselect`] to choose where the next window goes, or `dwindle togglesplit`.
