...
In the xref:guide.adoc#dwindle-layout[dwindle layout], `swap (left|down|up|right)` picks the window like `focus` does there:
a window that touches the focused one on that side, the most recently focused one if there are several.
