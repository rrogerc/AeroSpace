...
In the xref:guide.adoc#dwindle-layout[dwindle layout], `focus (left|down|up|right)` from a tiling window works like Hyprland's:
it goes to a window that touches the focused one on that side, never diagonally across a split.
If several windows do, the most recently focused one wins.
If no tiling window is that way, the command works as described above.
